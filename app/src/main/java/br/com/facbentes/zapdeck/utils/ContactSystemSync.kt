...
object ContactSystemSync {
    ...
    fun fetchSystemContacts(context: Context): List<ContactEntity> {
        ...
        for ((rawId, tc) in tempContacts) {
            ...
            var finalCreatedAt = rawId * 1000L
            ...
            if (isAppContactMarker(tc.rawNote)) {
                ...
                for (line in rawLines) {
                    ...
                    if (trimLine.startsWith("CreatedAt:")) {
                        val parsedTime = trimLine.substringAfter("CreatedAt:").trim().toLongOrNull()
                        if (parsedTime != null && parsedTime > 0) {
                            finalCreatedAt = parsedTime
                        }
                    } else if (trimLine.startsWith("LandlinePhone:")) {
        ...
        // Keep most recently added contacts at the top of the list
        contactList.sortWith(compareByDescending<ContactEntity> { it.createdAt }.thenByDescending { it.id })
